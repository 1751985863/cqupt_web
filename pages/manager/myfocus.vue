    ...
    <div class="focus-list"> 
     <div class="myfocus-list"> 
      <ul class="sui-nav nav-tabs nav-large"> 
       <li class="active"><a href="#one" data-toggle="tab">我的关注</a></li> 
      </ul> 
        ...
        <ul class="focuspro-list"> 
         <li class="pro-item" v-for="(item,index) in items" :key="index"> 
          ...
          <div class="fr btns"> 
           ...
           <button class="sui-btn btn-danger btn-cancel btn-bordered" @click="jumpNofollow(item.nickname)">取消关注</button> 
          </div> 
...
export default {
  ...
  created(){
      return  userApi.myfollow().then( res => {
          this.items= res.data.data 
       })
    },
    methods: {
         jumpNofollow(nikename){
             this.$confirm('确定要取消关注他吗', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(()=>{
          userApi.delFollow(nikename).then(res=>{
             this.$message({
                  message: res.data.message,
                  type: (res.data.flag?'success':'error')
              })
              userApi.myfollow().then( res => {
          this.items= res.data.data 
       })
          })
